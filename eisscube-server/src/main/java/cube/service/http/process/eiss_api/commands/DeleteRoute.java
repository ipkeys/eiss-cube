package cube.service.http.process.eiss_api.commands;

import com.google.gson.Gson;
import dev.morphia.Datastore;
import dev.morphia.query.Query;
import cube.db.Cube;
import cube.json.messages.commands.Command;
import cube.json.messages.commands.CommandIdRequest;
import cube.json.messages.commands.CommandResponse;
...
public class DeleteRoute implements Handler<RoutingContext> {
    ...
    private CommandResponse deleteCommand(CommandIdRequest req) {
        ...
        if (ObjectId.isValid(id)) {
            ...
            CubeCommand c = command.findAndDelete();
            if (c != null) {
                rc.setCommand(
                    Command.builder()
